...
const totalContributions = GAMES_JSON.reduce( (acc, game) => {
    return acc + Number(game.backers || 0);
}, 0);
...
const totalRaised = GAMES_JSON.reduce( (acc, game) => {
    return acc + Number(game.pledged || 0);
}, 0);
...
let displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised to fund ${totalGames === 1 
    ? `${totalGames} game.` 
    : `${totalGames} games.`} Currently, ${totalUnfunded === 1 
    ? `${totalUnfunded} game remains unfunded.` 
    : `${totalUnfunded} games remain unfunded.`} We need your help to fund these amazing games!`;
...
if (firstGame) {
    ...
    p1.textContent = firstGame.name
    firstGameContainer.appendChild(p1)
}

if (secondGame) {
    const p2 = document.createElement('p');
    p2.textContent = secondGame.name
